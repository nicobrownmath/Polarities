    ...
    public class WingedStarMinion : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (target == -1)
            {
                ...
                {
                    Dust dust = Dust.NewDustPerfect(Projectile.Center + new Vector2(0f, Projectile.height * 0.2f) + Main.rand.NextVector2CircularEdge(Projectile.width, Projectile.height * 0.6f) * (0.3f + Main.rand.NextFloat() * 0.5f), 228, (Vector2?)new Vector2(0f, (0f - Main.rand.NextFloat()) * 0.3f - 1.5f), 127, default(Color), 1f);
                    dust.scale = 0.5f;
                    dust.fadeIn = 1.1f;
                    dust.noGravity = true;
                    dust.noLight = true;
        ...
        private float wingRotationOffset;
        private Vector2 wingScale;
