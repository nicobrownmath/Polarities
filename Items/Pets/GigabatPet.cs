...
namespace Polarities.Items.Pets
{
	...
	public class GigabatPetBuff : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex)
		{
			...
			{
				Projectile.NewProjectile(player.GetSource_Buff(buffIndex), player.position.X + (float)(player.width / 2), player.position.Y + (float)(player.height / 2), 0f, 0f, ProjectileType<GigabatPetLarge>(), 0, 0f, player.whoAmI, 0f, 0f);
			}
			petProjectileNotSpawned = player.ownedProjectileCounts[ProjectileType<GigabatPetMedium>()] <= 0;
			if (petProjectileNotSpawned && player.whoAmI == Main.myPlayer)
			{
				Projectile.NewProjectile(player.GetSource_Buff(buffIndex), player.position.X + (float)(player.width / 2), player.position.Y + (float)(player.height / 2), 0f, 0f, ProjectileType<GigabatPetMedium>(), 0, 0f, player.whoAmI, 0f, 0f);
			}
			petProjectileNotSpawned = player.ownedProjectileCounts[ProjectileType<GigabatPetSmall>()] <= 0;
			if (petProjectileNotSpawned && player.whoAmI == Main.myPlayer)
			{
				Projectile.NewProjectile(player.GetSource_Buff(buffIndex), player.position.X + (float)(player.width / 2), player.position.Y + (float)(player.height / 2), 0f, 0f, ProjectileType<GigabatPetSmall>(), 0, 0f, player.whoAmI, 0f, 0f);
			}
		}
	}
